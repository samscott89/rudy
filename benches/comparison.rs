...
const TEST_ADDRESSES: &[u64] = &[
    0x100001000, 0x100002000, 0x100003000, 0x100004000, 0x100005000,
    0x100006000, 0x100007000, 0x100008000, 0x100009000, 0x10000a000,
];
...
"#, binary_path, format!("{:?}", TEST_ADDRESSES));
...
fn print_results(results: &[BenchmarkResult]) {
    ...
    for result in results {
        println!("📦 {}", result.name);
        println!("  Cold start:    {:>8.2} ms", result.cold_start.as_millis());
        println!("  Warm queries:  {:>8.2} ms ({:.1} ms/query)", 
            result.warm_queries.as_millis(),
        ...
        if result.name != baseline.name {
            ...
            let warm_speedup = baseline.warm_queries.as_secs_f64() / result.warm_queries.as_secs_f64();
