...
use crate::{file::SourceFile, DwarfDb};
...
impl<'db> Die<'db> {
    ...
    pub fn cu(&self, db: &'db dyn DwarfDb) -> CompilationUnitId<'db> {
    ...
    pub fn children(&self, db: &'db dyn DwarfDb) -> Result<Vec<Die<'db>>> {
    ...
    pub fn make_error<E: Into<anyhow::Error>>(
        &self,
        db: &'db dyn DwarfDb,
        error: E,
    ) -> DieAccessError {
    ...
    pub fn location(&self, db: &'db dyn DwarfDb) -> String {
        format!(
            "{} {:#010x}",
    ...
    pub fn format_with_location<T: AsRef<str>>(&self, db: &'db dyn DwarfDb, message: T) -> String {
        format!(
            "{} for {} {:#010x}",
            message.as_ref(),
            self.file(db).name(db),
    ...
    pub fn tag(&self, db: &'db dyn DwarfDb) -> gimli::DwTag {
    ...
    pub fn get_member(&self, db: &'db dyn DwarfDb, name: &str) -> Result<Die<'db>> {
        self.children(db)?
            .into_iter()
    ...
    pub fn get_member_by_tag(&self, db: &'db dyn DwarfDb, tag: gimli::DwTag) -> Result<Die<'db>> {
        self.children(db)?
            .into_iter()
    ...
    pub fn get_generic_type_entry(&self, db: &'db dyn DwarfDb, name: &str) -> Result<Die<'db>> {
        self.children(db)?
            .into_iter()
            .find(|child| {
    ...
    pub fn string_attr(&self, db: &'db dyn DwarfDb, attr: gimli::DwAt) -> Result<String> {
        self.with_entry_and_unit(db, |entry, unit_ref| {
            parse_die_string_attribute(entry, attr, unit_ref).into_die_result(db, self)
        })?
    }

    pub fn udata_attr(&self, db: &'db dyn DwarfDb, attr: gimli::DwAt) -> Result<usize> {
    ...
    pub fn print(&self, db: &'db dyn DwarfDb) -> String {
        self.with_entry_and_unit(db, |entry, unit_ref| {
    ...
    pub fn unit_ref(&self, db: &'db dyn DwarfDb) -> Result<UnitRef<'db>> {
        self.cu(db)
            .unit_ref(db)
...
pub fn position<'db>(db: &'db dyn DwarfDb, entry: Die<'db>) -> Result<Option<Position<'db>>> {
