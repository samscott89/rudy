...
This demo shows how to use the RDI-LLDB extension for enhanced Rust debugging.
...
1. **Install the extension:**
   ...
   - Add `command script import <path to repo>/python/rdi_lldb.py` to `~/.lldbinit`
...
(lldb) rdi eval session.user.name
Error: not implemented yet
```
...
- ✅ RDI server starts automatically
...
You can test the protocol manually:

```bash
# Start the server manually
cargo run -p rdi-lldb --bin rdi-lldb-server

# In another terminal, test with the Python client
python3 python/rdi_lldb_client.py target/debug/lldb_demo
...
```
LLDB → Python Extension → TCP Socket → RDI Server → rust-debuginfo
  ↑                                           ↓
