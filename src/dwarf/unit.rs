...
pub fn get_unit_ref<'db>(
    ...
    fn get_unit<'db>(
        ...
        let dwarf = get_dwarf(db, file_id)?;

        let mut units = dwarf.units();
        while let Ok(Some(header)) = units.next() {
            if header.offset() == cu_offset {
                return Some(DwarfUnit {
                    file_path: file_id.full_path(db).to_string(),
                    offset: cu_offset,
    ...
    let unit = &get_unit(db, file_id, cu_offset).as_ref()?.unit;
    let dwarf = get_dwarf(db, file_id)?;
...
pub struct UnitRef<'a, R: gimli::Reader = DwarfReader> {
    ...
    pub dwarf: crate::database::MappedRef<'a, gimli::Dwarf<R>>,
